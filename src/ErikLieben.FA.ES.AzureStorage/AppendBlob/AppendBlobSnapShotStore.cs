...
/// <param name="settings">The Append Blob settings controlling container creation and defaults.</param>
public partial class AppendBlobSnapShotStore(
    ...
    : ISnapShotStore
{
    ...
    public async Task SetAsync(IBase @object, JsonTypeInfo jsonTypeInfo, IObjectDocument document, int version, string? name = null, CancellationToken cancellationToken = default)
    {
        ...
        using var activity = FaesInstrumentation.Storage.StartActivity("AppendBlobSnapShotStore.Set");
    ...
    public async Task<T?> GetAsync<T>(JsonTypeInfo<T> jsonTypeInfo, IObjectDocument document, int version, string? name = null, CancellationToken cancellationToken = default) where T : class, IBase
    {
        ...
        using var activity = FaesInstrumentation.Storage.StartActivity("AppendBlobSnapShotStore.Get");
        if (activity?.IsAllDataRequested == true)
        {
            activity.SetTag(FaesSemanticConventions.DbSystem, FaesSemanticConventions.DbSystemAzureAppendBlob);
    ...
    {
        using var activity = FaesInstrumentation.Storage.StartActivity("AppendBlobSnapShotStore.List");
        ArgumentNullException.ThrowIfNull(document);
    ...
    public async Task<bool> DeleteAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var activity = FaesInstrumentation.Storage.StartActivity("AppendBlobSnapShotStore.Delete");
        ArgumentNullException.ThrowIfNull(document);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var activity = FaesInstrumentation.Storage.StartActivity("AppendBlobSnapShotStore.DeleteMany");
        if (activity?.IsAllDataRequested == true)
        {
            activity.SetTag(FaesSemanticConventions.DbSystem, FaesSemanticConventions.DbSystemAzureAppendBlob);
            activity.SetTag(FaesSemanticConventions.DbOperation, FaesSemanticConventions.DbOperationDelete);
