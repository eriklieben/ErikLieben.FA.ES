...
/// </summary>
public partial class AppendBlobDocumentTagStore : IDocumentTagStore
{
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<string, bool> VerifiedContainers = new(StringComparer.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="AppendBlobDocumentTagStore"/> class.
    /// </summary>
    /// <param name="clientFactory">The Azure client factory used to create <see cref="BlobServiceClient"/> instances.</param>
    /// <param name="defaultDocumentTagType">The default tag provider type (e.g., "appendblob").</param>
    ...
    public AppendBlobDocumentTagStore(
    ...
    public async Task SetAsync(IObjectDocument document, string tag)
    {
        ...
        // Download the document with the same tag, so that we're sure it's not overridden in the meantime
