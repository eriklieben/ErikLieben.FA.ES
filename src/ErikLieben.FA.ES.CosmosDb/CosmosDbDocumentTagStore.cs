...
public partial class CosmosDbDocumentTagStore : IDocumentTagStore
{
    ...
    /// <summary>
    /// Removes the specified tag from the given document by deleting the tag entity from CosmosDB.
    /// </summary>
    ...
    public async Task RemoveAsync(IObjectDocument document, string tag)
    {
        ...
        var sanitizedTag = SanitizeTag(tag);
        var tagKey = CosmosDbTagEntity.CreateTagKey(document.ObjectName, sanitizedTag);
        var id = CosmosDbTagEntity.CreateId("document", document.ObjectName, sanitizedTag, document.ObjectId);
        var partitionKey = new PartitionKey(tagKey);
