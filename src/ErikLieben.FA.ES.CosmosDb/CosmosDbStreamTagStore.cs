...
/// <summary>
/// Provides a CosmosDB-backed store for associating tags with event streams.
/// Uses the same tags container with different tag type discriminator.
/// </summary>
public partial class CosmosDbStreamTagStore : IDocumentTagStore
{
    ...
    public async Task SetAsync(IObjectDocument document, string tag)
    {
        ...
        {
            Id = CosmosDbTagEntity.CreateId("stream", document.ObjectName, sanitizedTag, document.ObjectId),
            TagKey = tagKey,
            TagType = "stream",
    ...
    public async Task<IEnumerable<string>> GetAsync(string objectName, string tag)
    {
        ...
        var query = new QueryDefinition(
            "SELECT c.objectId FROM c WHERE c.tagKey = @tagKey AND c.tagType = 'stream'")
