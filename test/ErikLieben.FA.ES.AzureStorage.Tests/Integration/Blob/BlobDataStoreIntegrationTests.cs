...
/// <summary>
/// Integration tests for BlobDataStore using Azurite TestContainer.
/// Tests event stream read/append operations against real blob storage.
/// </summary>
[Collection("AzuriteIntegration")]
[Trait("Category", "Integration")]
[Trait("Feature", "BlobStorage")]
public class BlobDataStoreIntegrationTests : IAsyncLifetime
{
    private readonly AzuriteContainerFixture _fixture;
    private readonly string _testId;
    private BlobDataStore? _dataStore;

    public BlobDataStoreIntegrationTests(AzuriteContainerFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task InitializeAsync()
    {
        ...
        _dataStore = new BlobDataStore(blobClientFactory, autoCreateContainer: true);

        // Pre-create container for tests
        var containerName = "testobject";
        var container = _fixture.BlobServiceClient!.GetBlobContainerClient(containerName);
    ...
    [Fact]
    public async Task Should_preserve_event_timestamps_when_requested()
    {
        // Arrange
        var streamId = $"stream-{_testId}-timestamp";
        var document = CreateObjectDocument(streamId);
        var originalTimestamp = DateTimeOffset.UtcNow.AddDays(-30);

        var eventWithTimestamp = new BlobJsonEvent
        {
        ...
        Assert.NotNull(events);
        Assert.Single(events);
        // The timestamp should be within a reasonable range of the original
        var blobEvent = Assert.IsType<BlobJsonEvent>(events[0]);
        Assert.True(Math.Abs((blobEvent.Timestamp - originalTimestamp).TotalSeconds) < 2,
            $"Expected timestamp close to {originalTimestamp}, but got {blobEvent.Timestamp}");
    }
    ...
    public async Task Should_handle_large_payloads()
    {
        ...
        var document = CreateObjectDocument(streamId);
        var largePayload = new string('x', 50000); // 50KB payload
